...
Adafruit_MQTT_Subscribe Speed = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/Speed");
Adafruit_MQTT_Subscribe Bright = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/Bright");
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(500))) {
    if (subscription == &Red) {
      ...
      Serial.println(redCol);
      analogWrite(4,redCol);
    }
    else if (subscription == &Blue) {
      Serial.print(F("Blue: "));
      blueCol = atoi((char *)Blue.lastread);
      Serial.println(blueCol);
      analogWrite(5,blueCol);
    }
    else if (subscription == &Green) {
      Serial.print(F("Green: "));
      greenCol = atoi((char *)Green.lastread);
      Serial.println(greenCol);
      analogWrite(14,greenCol);
    }
    else if (subscription == &Speed) {
      Serial.print(F("delayBlink: "));
      delayBlink = atoi((char *)Speed.lastread);
      Serial.println(delayBlink);
    }
    else if (subscription == &Bright) {
      Serial.print(F("Bright: "));
      maxBright = atoi((char *)Bright.lastread);
